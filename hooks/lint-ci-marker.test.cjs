 ...
 *
 * Coverage (plan <behavior> — every 04-PATTERNS.md scenario):
 *   - git push with a valid marker + clean tree → allow
 *   - git push with NO marker → deny; reason names `lint-ci-stamp` + cites ENF-05
 *   - git push with a DIRTY tree → deny (EP-4), even though the marker exists
 *   - gh pr create with a valid marker → allow (same gate, pr-create trigger)
...
// Default deps: a clean worktree, a marker present for the current staged tree SHA, no
// override. Tests override individual readers per scenario.
function deps(over = {}) {
