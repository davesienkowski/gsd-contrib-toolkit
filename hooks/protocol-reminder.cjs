...
function buildReminder() {
  return [
    ...
    '',
    '  ⚠ P1–P3 are now ENFORCED (ENF-19, CTK-ADR-0004). On a contribution branch they each',
    '  need an ARTIFACT in `.gsd/contrib/<branch-slug>/`, written AS you do the step — the',
    '  gate reads them at the moment you file, push, or open the PR:',
    '      P1-repro.json    before `gh issue create` — mechanism + reproduced + OBSERVED output',
    '      P2-review.json   before `gh pr create`    — quoted ADRs + a disposition-with-proof',
    '                                                  per finding (fixed | not-a-defect | filed)',
    '      P3-red.json      before `git push`        — the test FAILING first, then green',
    '      P3-matrix.json   before `gh pr create`    — a `gsd-test -base next -head HEAD` run id',
    '                                                  (read LIVE; must be green and newer than HEAD)',
    '  Docs-only branch? Record `{"not_applicable":{"reason":"..."}}` — but it is checked against',
    '  the real diff, so it is refused if the branch touched src/ tests/ bin/ scripts/ hooks/.',
