...
test('INST-03(a): a sandbox install wires EVERY CAP_MARKER target to a real file (13 resolve, no dangling)', { skip: SKIP }, () => {
  ...
  try {
    ...
    // (a) NO DANGLING: enumerate every tagged wired target and stat each — all 13 resolve.
    const targets = wiredTargets(sb.settingsPath);
    assert.strictEqual(targets.length, 13, 'install must wire EXACTLY 13 tagged targets, got ' + targets.length);
    ...
    // Cross-check via the driver's own fail-loud verifier: verified === 13, no throw.
    ...
    assert.strictEqual(verification.verified, 13, 'verifyWiredTargets must confirm all 13 wired targets resolve');
  } finally {
    sb.dispose();
  }
