...
**Recovery Offramp.** If a gate **denies** an action during the sweep/re-review (or the skill surfaces a real blocking issue mid-triage), don't dead-stop and don't route around it: the deny stays **fail-closed/unbypassable** and this offramp is **advisory only** — it NEVER bypasses the gate or uses `GSD_CONTRIB_OVERRIDE` to dodge a real failure. Take one of two tracked paths, then return to the sweep once it's green: **`/gsd-quick`** for a trivial inline fix, or **`/gsd-debug`** (or `/gsd-discuss-phase`→`/gsd-plan-phase`→`/gsd-execute-phase`) for a tracked, resumable one. See the fuller **Recovery Offramp** section in the `core-contribution` skill for the full version.

**Authoring hand-off.** The moment the sweep stops *adjudicating* and starts **authoring** — a fix for a defect you surfaced, a correction pushed to a stalled PR, a follow-up issue, any issue/PR opened on `open-gsd/gsd-core` — stop and route to **`/gsd-submit`** (the `core-contribution` skill), never to a bare `gh issue create` (the skill carries the security routing that sends a real exploitable vuln to the private advisory instead of a public issue). Carry the hand-off packet: the related `#number`s, the evidence you actually captured with its provenance (and an explicit "not run" for anything you didn't), the design-record clauses you quoted, the RT0 verdict, the ball-in-court finding, and any scope fence. Full packet in the **Authoring hand-off** section of the `maintainer-review-sweep` skill. Like the offramp above, this is **model-driven/advisory — not new enforcement and no new hook**: the ENF-21 gate's **fail-closed** deny on `gh issue create` / `gh pr create` / `git push` is the actual floor, and this only tells you where to go instead of dead-ending at it.
