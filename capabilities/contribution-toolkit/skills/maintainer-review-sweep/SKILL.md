...
- **Confirming a bug → emit the Agent Brief (mirror trek-e).** When triage confirms a bug and moves it to `confirmed-bug` + `ready-for-agent`, don't just relabel — post the structured **Agent Brief** an AFK executor can act on: *Reproduction (empirical, on live `next`) → Root cause (`file:line`) → Fix → Acceptance criteria → Verify*. The brief IS the expensive artifact the `ready-for-agent` label gates dispatch on. Format + verified examples (#2070, #2118) in [triage-assist.md](triage-assist.md).
