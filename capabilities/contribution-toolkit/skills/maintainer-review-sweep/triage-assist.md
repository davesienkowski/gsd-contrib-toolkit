...
## Root-cause with the graph (memtrace) when confirming a bug

trek-e's triage Defect Lane root-causes via **memtrace before grep** — the same graph tools the sweep uses (re-review.md 8a). When you confirm a bug and need the `file:line` root cause for the Agent Brief: `find_code` / `find_symbol` from the error/symptom text → `get_symbol_context` (callers/callees) → `get_impact` (blast radius) → `get_timeline` (did a specific commit introduce it?) → the co-change pass (`git log` on the changed files, or `get_cochange_context` where your build exposes it) for the coupled symbols the fix must also touch — feeds the brief's Fix step so it doesn't ship half a change. ⚠️ **Same environment caveats as the sweep** (`core-contribution` `reference.md`, *Memtrace for code lookup — in THIS environment*): `get_impact` is a **FLOOR** for engine internals (grep-confirm callers), omit a path-shaped `repo_id`, memtrace is main-session / granted-agent only (an ungranted subagent greps), and not every verb trek-e names exists here — the chain above is the confirmed-available subset; probe before using a richer verb. Cite the real source (`src/*.cts`, never the generated `bin/lib/*.cjs`, ADR-457); a grep fallback is honest as long as you name the graph rung that failed. **Reproduction stays empirical** — the graph explains *why*, it never substitutes for the actual run in the brief's Reproduction block (honesty-of-evidence).
