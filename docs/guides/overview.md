...
| Layer | What | Where |
|---|---|---|
| **Enforcement** | **18 harness-run hook registrations across 17 hook scripts** — 15 fail-closed `PreToolUse` gates (14 on `Bash`, 1 on `Write`/`Edit`) + 1 advisory `UserPromptSubmit` reminder + 1 observability recorder wired on both `PostToolUse` and `PostToolUseFailure` (the one hook registered twice). Only the 15 `PreToolUse` gates block. | `hooks/` (+ shared anti-bypass `hooks/lib/`) |
...
| **Share form** | An opt-in, consent + ledger tracked GSD capability bundling the 17 hook scripts + 2 skills + 5 commands | `capabilities/contribution-toolkit/` |
...
- **Contributor pillar (hard enforcement).** Drive a contribution with **`/gsd-submit`** (+ the
  `gsd-core-contribution` skill). Every wired `PreToolUse` gate makes a broken issue/PR/push
  *impossible to land*. →
